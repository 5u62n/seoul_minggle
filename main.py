...
async def recommend(theme: str, age: str, gender: str):
    ...
    KEY = str(key_info['dataseoul'])
    
    AREA_LIST = ['강남역', '홍대입구역(2호선)', '성수카페거리', '이태원역', '가산디지털단지역'] # 예시 리스트
    rows = [fetchAreaData(KEY, area) for area in AREA_LIST]
    df = pd.DataFrame([r for r in rows if r])

    if df.empty: return {"error": "No data available"}

    # 2. 실시간 정규화 수행 (이 부분이 꼭 필요합니다!)
    ...
    return result[['AREA_NM', 'FINAL_SCORE']].to_dict(orient="records")